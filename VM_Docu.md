...
I checked my NIC settings, but they were fine. I then ran \"sudo
...
but there were none. The interface name was "eth0"

While still in Kali, I then used the Network Manager Command Line
...
configuration file with "sudo nano /etc/network/interfaces."
...
made with "sudo systemctl restart networking."
...
I unloaded the required kernel modules with \"sudo modprobe -r
nf_nat_ftp nf_conntrack_ftp sudo modprobe -r bridge."

I reloaded the libvert service with "sudo systemctl restart libvirtd."

Finally, I started the virtual network, rebuilding the virbr0 bridge
with "sudo virsh net-start default
...
From there, I updated the system \"sudo apt update

sudo apt full-upgrade -y."

I then changed the password with "passwd."
...
And checked the status with "sudo systemctl status ssh."

I also verified if it was listening with "sudo ss -tulpn \| grep :22."

After, I got my IP with "ip a,"

and I used that to check port status. First though, I installed nmap
with "sudo apt install nmap -y."

After that, I checked the open ports with "nmap -sV 192.168.122.209/24"
